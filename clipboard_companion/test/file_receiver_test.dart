...
void main() {
  ...
  test('happy path reassembles the file and removes the temp file', () async {
    ...
    final t = await prepareTransfer(data: data, chunkSize: 4, peerId: peerId, key: key);

    expect((await receiver.handleOffer(t.offer)).statusCode, 200);
    for (var i = 0; i < t.chunks.length; i++) {
      expect((await receiver.handleChunk(t.transferId, i, t.chunks[i])).statusCode, 200);
    }
    final finish = await sealControl(FileFinishPayload(t.transferId).toJson(), peerId, key);
  ...
  test('corrupted chunk tears the session down with 400', () async {
    ...
    corrupt[0] ^= 0xFF;
    expect((await receiver.handleChunk(t.transferId, 0, corrupt)).statusCode, 400);
    expect((await receiver.handleChunk(t.transferId, 1, t.chunks[1])).statusCode, 404);
  });

  test('hash mismatch returns 422 and deletes the temp file', () async {
    final (receiver, destDir, _) = await makeReceiver();
    final t = await prepareTransfer(
        data: data, chunkSize: 4, peerId: peerId, key: key, overrideHash: '0' * 64);
    await receiver.handleOffer(t.offer);
    for (var i = 0; i < t.chunks.length; i++) {
  ...
  test('cancel removes the session; later chunks get 410', () async {
    ...
    await receiver.handleOffer(t.offer);
    await receiver.handleChunk(t.transferId, 0, t.chunks[0]);
    final cancel = await sealControl(FileCancelPayload(t.transferId).toJson(), peerId, key);
    expect((await receiver.handleCancel(cancel)).statusCode, 200);
    expect((await receiver.handleChunk(t.transferId, 1, t.chunks[1])).statusCode, 410);
  });
  ...
  test('chunk for an unknown transfer returns 404', () async {
    final (receiver, _, _) = await makeReceiver();
    expect((await receiver.handleChunk('nope', 0, List<int>.filled(32, 0))).statusCode, 404);
  });
  ...
  test('a duplicate active transferId is rejected with 409', () async {
    final (receiver, _, _) = await makeReceiver();
    final t = await prepareTransfer(data: [1, 2, 3, 4], chunkSize: 4, peerId: peerId, key: key);
    expect((await receiver.handleOffer(t.offer)).statusCode, 200);
  ...
  test('colliding file names get a unique suffix', () async {
    ...
    final finish = await sealControl(FileFinishPayload(t.transferId).toJson(), peerId, key);
    expect((await receiver.handleFinish(finish)).statusCode, 200);
    expect(await File('${destDir.path}/file (1).bin').readAsBytes(), [1, 2, 3, 4]);
  });
}
