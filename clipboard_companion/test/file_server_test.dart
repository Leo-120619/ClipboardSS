...
void main() {
  ...
  Future<ClipServer> makeServer({required bool withReceiver}) async {
    ...
    final identity = DeviceIdentity(id: 'cccccccc-0000-0000-0000-000000000000', name: 'Companion');
    ...
    return ClipServer(
      identity: identity,
      pairingCoordinator: PairingCoordinator(identity: identity, pairedStore: store),
      pairedStore: store,
      onClipReceived: (_) {},
      fileReceiver: receiver,
    );
  ...
  test('file routes 404 when no FileReceiver is wired', () async {
    ...
    final response = await handler(Request('POST', uri('/v1/file/chunk'),
        headers: {'x-transfer-id': 't', 'x-chunk-index': '0'}, body: <int>[0]));
  ...
  test('offer route accepts a sealed envelope and returns ready', () async {
    ...
    final response = await handler(Request('POST', uri('/v1/file/offer'),
        body: '{"v":${t.offer.v},"sourceDeviceId":"${t.offer.sourceDeviceId}",'
  ...
  test('chunk route parses headers and stores the chunk', () async {
    ...
    await handler(Request('POST', uri('/v1/file/offer'),
        body: '{"v":${t.offer.v},"sourceDeviceId":"${t.offer.sourceDeviceId}",'
            '"nonce":"${t.offer.nonce}","ciphertext":"${t.offer.ciphertext}"}'));
